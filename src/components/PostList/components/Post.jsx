...
const Post = ({ post, isFirstPost }) => {
  ...
  const handleHover = debounce(() => {
    console.log('Mouse has hovered for more than 2 second!');
    setOpenProfile(true);
  }, 2000);
  ...
  return (
    <Wrapper>
      ...
      <Content>
        <Header onClick={() => navigate(`/users/${post.author?._id}`)}>
          <AuthorImg src={post.author.picture.url} alt={post.author.username} />
          <AuthorMeta>
            <Tippy interactive content={<AuthorDetail author={post.author} />}>
              <AuthorName
                ref={authorRef}
        ...
        <Title onClick={() => navigate(`posts/${post.id}`)}>{post.title}</Title>
        <TagList>
          <Tag>#career</Tag>
          <Tag href='#!'>
